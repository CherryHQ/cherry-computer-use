...
| 区域 | 评分 | 原因 | 下一步 |
| --- | --- | --- | --- |
| TypeScript SDK | C | 类型化客户端、协议校验、ESM/CJS tarball 与三端生命周期通过本机验证；Windows 11 ARM64/Linux X11 的真实 SDK 观察、截图和语义点击通过。 | macOS GUI 待授权；macOS 权限申请与 Cherry 本地入口已接通；其余动作、Wayland、远端 CI、平台包与 Electron 完整桌面验收仍未完成。 |
